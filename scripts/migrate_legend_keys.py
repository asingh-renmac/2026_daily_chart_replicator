...
def main() -> int:
    ...
    for old, new in sorted(mapping.items()):
        if old not in store:
            ...
            continue
        if new in store:
            skipped.append((old, new))        # already migrated, or new key independently earned
            continue
    ...
    for old, new, copy in moves:
        ...
        if copy:
            print(f"        (old key still serves a phrase-only slot, so it stays)")
        print()
    ...
    store_path.write_text(json.dumps(store, indent=2, ensure_ascii=False),
                          encoding="utf-8")
    print(f"\nbackup : {backup}")
    print(f"wrote  : {store_path}  ({len(store)} entries, {len(moves)} moved)")
    return 0
